# Daily Variance Report — 2026-06-09
_Generated 2026-06-09T05:18:23Z · 15 games · significant deviations only_

## 1 · Roster / Injury Delta
- **SEA** [IL] — Seattle Mariners placed SS J.P. Crawford on the 10-day injured list retroactive to June 6, 2026. Right hand contusion.
- **SEA** [recalled] — Seattle Mariners recalled 2B Ryan Bliss from Tacoma Rainiers.
- **BAL** [IL] — Baltimore Orioles placed RHP Chris Bassitt on the 15-day injured list retroactive to June 5, 2026. Low back discomfort.
- **BAL** [claimed] — Chicago Cubs claimed RHP Eduarniel Núñez off waivers from Baltimore Orioles.
- **BAL** [recalled] — Baltimore Orioles recalled RHP Trey Gibson from Norfolk Tides.
- **LAD** [TRADE] — Los Angeles Dodgers traded LHP Antoine Kelly to Chicago Cubs for cash.
- **PIT** [selected] — Pittsburgh Pirates selected the contract of 3B Davis Wendzel from Indianapolis Indians.
- **PIT** [optioned] — Pittsburgh Pirates optioned RF Jhostynxon Garcia to Indianapolis Indians.
- **BOS** [optioned] — Boston Red Sox optioned LHP Joe La Sorsa to Worcester Red Sox.
- **BOS** [recalled] — Boston Red Sox recalled OF Nate Eaton from Worcester Red Sox.
- **TB** [IL] — Tampa Bay Rays transferred RHP Jonathan Heasley from the 15-day injured list to the 60-day injured list.
- **TB** [selected] — Tampa Bay Rays selected the contract of RF Austin Slater from Durham Bulls.
- **TB** [optioned] — Tampa Bay Rays optioned 3B Oliver Dunn to Durham Bulls.
- **MIN** [signed] — Minnesota Twins signed free agent RHP Failin Placencio to a minor league contract.
- **MIN** [TRADE] — Arizona Diamondbacks traded RHP Taylor Rashi to Minnesota Twins for cash.
- **AZ** [signed] — Arizona Diamondbacks signed free agent LF Max Kepler.
- **AZ** [activated] — Arizona Diamondbacks activated RHP Taylor Clarke.
- **AZ** [optioned] — Arizona Diamondbacks optioned LHP Philip Abner to Reno Aces.
- **AZ** [signed] — Arizona Diamondbacks signed free agent RHP Oscar Echegarreta to a minor league contract.
- **AZ** [TRADE] — Arizona Diamondbacks traded RHP Taylor Rashi to Minnesota Twins for cash.
- **TOR** [outrighted] — Toronto Blue Jays sent RHP Tanner Andrews outright to Buffalo Bisons.
- **TOR** [IL] — Toronto Blue Jays activated RHP Tommy Nance from the 15-day injured list.
- **TOR** [DFA] — Toronto Blue Jays designated RHP Yariel Rodríguez for assignment.
- **STL** [optioned] — St. Louis Cardinals optioned CF Victor Scott II to Memphis Redbirds.
- **TEX** [released] — Texas Rangers released LF Sam Haggerty.
- **TEX** [TRADE] — Texas Rangers traded SS Richie Martin Jr. to Colorado Rockies for cash.
- **CHC** [TRADE] — Los Angeles Dodgers traded LHP Antoine Kelly to Chicago Cubs for cash.
- **CHC** [claimed] — Chicago Cubs claimed RHP Eduarniel Núñez off waivers from Baltimore Orioles.
- **CHC** [selected] — Chicago Cubs selected the contract of LHP Antoine Kelly from Iowa Cubs.
- **COL** [TRADE] — Texas Rangers traded SS Richie Martin Jr. to Colorado Rockies for cash.
- **COL** [recalled] — Colorado Rockies recalled LHP Sammy Peralta from Albuquerque Isotopes.
- **COL** [optioned] — Colorado Rockies optioned RHP TJ Shook to Albuquerque Isotopes.
- **HOU** [IL] — Houston Astros placed SS Nick Allen on the 10-day injured list. Left hamstring strain.
- **HOU** [recalled] — Houston Astros recalled 3B Shay Whitcomb from Sugar Land Space Cowboys.
- **LAA** [IL] — Los Angeles Angels placed RHP Jack Kochanowicz on the 15-day injured list. Right elbow inflammation.
- **LAA** [outrighted] — Los Angeles Angels sent RHP Shaun Anderson outright to Salt Lake Bees.
- **LAA** [recalled] — Los Angeles Angels recalled LHP Sam Aldegheri from Salt Lake Bees.
- **LAA** [IL] — Los Angeles Angels placed 2B Adam Frazier on the 10-day injured list retroactive to June 6, 2026. Right elbow inflammation.
- **LAA** [IL] — Los Angeles Angels placed 2B Vaughn Grissom on the 10-day injured list retroactive to June 5, 2026. Left oblique strain.
- **LAA** [IL] — Los Angeles Angels transferred 3B Yoán Moncada from the 10-day injured list to the 60-day injured list. Right knee inflammation.
- **LAA** [recalled] — Los Angeles Angels recalled SS Denzer Guzman from Salt Lake Bees.
- **LAA** [selected] — Los Angeles Angels selected the contract of 1B Trey Mancini from Salt Lake Bees.
- **CIN** [IL] — Cincinnati Reds activated RHP Rhett Lowder from the 15-day injured list.
- **CIN** [optioned] — Cincinnati Reds optioned RHP Luis Mey to Louisville Bats.
- **WSH** [recalled] — Washington Nationals recalled RHP Riley Cornelio from Rochester Red Wings.
- **WSH** [optioned] — Washington Nationals optioned RHP Cole Henry to Rochester Red Wings.
- **WSH** [optioned] — Washington Nationals optioned RHP Cole Henry to Rochester Red Wings.
- **MIL** [IL] — Milwaukee Brewers placed RHP Carlos Rodriguez on the 15-day injured list retroactive to May 31, 2026.
- **MIL** [activated] — Milwaukee Brewers activated RHP Joel Kuhnel.
- **MIL** [outrighted] — Milwaukee Brewers sent RHP Jake Woodford outright to Nashville Sounds.
- **MIL** [optioned] — Milwaukee Brewers optioned RHP Craig Yoho to Nashville Sounds.
- **ATH** [IL] — Athletics activated 3B Max Muncy from the 10-day injured list.
- **ATH** [optioned] — Athletics optioned SS Darell Hernaiz to Las Vegas Aviators.

_Suppressed 2 minor/cosmetic move(s) (ASG/NUM); 0 unrecognized code-type(s) -> logs/unknown_tx_codes.log._

## 2 · Starting Pitcher Flags
- **LAD @ PIT** (home SP Paul Skenes): very high K% (28.0% vs lg 21.7)
- **BOS @ TB** (home SP Nick Martinez): very low K% (12.3% vs lg 21.7)
- **MIN @ DET** (away SP Taj Bradley): very high K% (28.7% vs lg 21.7)
- **PHI @ TOR** (home SP Dylan Cease): very high K% (36.1% vs lg 21.7)
- **CHC @ COL** (home SP Tomoyuki Sugano): very low K% (13.7% vs lg 21.7)
- **CIN @ SD** (away SP Chase Burns): very high K% (29.7% vs lg 21.7)

## 3 · Bullpen Fatigue (last 3 games)
- **SEA** Andrés Muñoz — 2 apps / 39 pitches over last 3 games — monitor
- **SEA** Matt Brash — 2 apps / 35 pitches over last 3 games — monitor
- **BAL** Albert Suárez — 2 apps / 68 pitches over last 3 games — monitor
- **TB** Casey Legumina — 2 apps / 36 pitches over last 3 games — monitor
- **TB** Kevin Kelly — 2 apps / 37 pitches over last 3 games — monitor
- **TB** Garrett Cleavinger — 2 apps / 36 pitches over last 3 games — monitor
- **MIA** Michael Petersen — 2 apps / 43 pitches over last 3 games — monitor
- **MIA** Anthony Bender — 2 apps / 46 pitches over last 3 games — monitor
- **NYY** David Bednar — 2 apps / 38 pitches over last 3 games — monitor
- **TOR** Adam Macko — 2 apps / 37 pitches over last 3 games — monitor
- **TOR** Simeon Woods Richardson — threw 48 pitches last night — likely unavailable
- **STL** Riley O'Brien — 2 apps / 38 pitches over last 3 games — monitor
- **ATL** Tyler Kinley — 2 apps / 42 pitches over last 3 games — monitor
- **CHC** Jacob Webb — 2 apps / 39 pitches over last 3 games — monitor
- **CHC** Daniel Palencia — 2 apps / 35 pitches over last 3 games — monitor
- **LAA** Samy Natera Jr. — 2 apps / 40 pitches over last 3 games — monitor
- **LAA** Ryan Zeferjahn — 2 apps / 59 pitches over last 3 games — monitor
- **CIN** Sam Moll — 2 apps / 35 pitches over last 3 games — monitor
- **SD** Mason Miller — 2 apps / 36 pitches over last 3 games — monitor
- **WSH** Miles Mikolas — threw 54 pitches last night — likely unavailable
- **WSH** Mitchell Parker — 2 apps / 41 pitches over last 3 games — monitor
- **SF** Keaton Winn — overused — 3 apps / 63 pitches over last 3 games
- **SF** Erik Miller — 2 apps / 38 pitches over last 3 games — monitor

## 4 · Lineup / Platoon
- SEA @ BAL — BAL faces RHP (Logan Gilbert)
- SEA @ BAL — SEA faces LHP (Trevor Rogers)
- LAD @ PIT — PIT faces LHP (Eric Lauer)
- LAD @ PIT — LAD faces RHP (Paul Skenes)
- BOS @ TB — TB faces LHP (Payton Tolle)
- BOS @ TB — BOS faces RHP (Nick Martinez)
- MIN @ DET — DET faces RHP (Taj Bradley)
- MIN @ DET — MIN faces RHP (Troy Melton)
- AZ @ MIA — MIA faces RHP (Zac Gallen)
- AZ @ MIA — AZ faces RHP (Max Meyer)
- NYY @ CLE — CLE faces RHP (Gerrit Cole)
- NYY @ CLE — NYY faces RHP (Slade Cecconi)
- PHI @ TOR — TOR faces RHP (Zack Wheeler)
- PHI @ TOR — PHI faces RHP (Dylan Cease)
- STL @ NYM — NYM faces RHP (Dustin May)
- STL @ NYM — STL faces RHP (Freddy Peralta)
- TEX @ KC — KC faces RHP (Nathan Eovaldi)
- TEX @ KC — TEX faces RHP (Stephen Kolek)
- ATL @ CWS — CWS faces RHP (Grant Holmes)
- ATL @ CWS — ATL faces RHP (Erick Fedde)
- CHC @ COL — COL faces RHP (Colin Rea)
- CHC @ COL — CHC faces RHP (Tomoyuki Sugano)
- HOU @ LAA — LAA faces RHP (Kai-Wei Teng)
- HOU @ LAA — HOU faces RHP (Walbert Ureña)
- CIN @ SD — SD faces RHP (Chase Burns)
- CIN @ SD — CIN faces RHP (Lucas Giolito)
- WSH @ SF — SF faces LHP (Andrew Alvarez)
- WSH @ SF — WSH faces RHP (Adrian Houser)
- MIL @ ATH — ATH faces LHP (Robert Gasser)
- MIL @ ATH — MIL faces RHP (J.T. Ginn)

