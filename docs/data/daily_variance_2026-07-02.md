# Daily Variance Report — 2026-07-02
_Generated 2026-07-01T16:43:22Z · 9 games · significant deviations only_

## 1 · Roster / Injury Delta
- **PIT** [signed] — Pittsburgh Pirates signed free agent SS Ryan McCarty to a minor league contract.
- **PHI** [optioned] — Philadelphia Phillies optioned RHP Chase Shugart to Lehigh Valley IronPigs.
- **PHI** [selected] — Philadelphia Phillies selected the contract of RHP Lou Trivino from Lehigh Valley IronPigs.
- **CIN** [IL] — Cincinnati Reds activated RHP Emilio Pagán from the 15-day injured list.
- **CIN** [IL] — Cincinnati Reds placed CF Dane Myers on the 10-day injured list. Left shoulder contusion.
- **CIN** [selected] — Cincinnati Reds selected the contract of 2B Ivan Johnson from Louisville Bats.
- **CIN** [optioned] — Cincinnati Reds optioned RHP Zach McCambley to Louisville Bats.
- **MIL** [optioned] — Milwaukee Brewers optioned LHP Robert Gasser to ACL Brewers.
- **MIL** [selected] — Milwaukee Brewers selected the contract of RHP Garrett Stallings from Nashville Sounds.
- **MIA** [IL] — Miami Marlins placed RHP Anthony Bender on the 15-day injured list. Right shin stress reaction.
- **MIA** [IL] — Miami Marlins activated C Liam Hicks from the 10-day injured list.
- **MIA** [optioned] — Miami Marlins optioned 3B Graham Pauley to Jacksonville Jumbo Shrimp.
- **MIA** [recalled] — Miami Marlins recalled RHP Zach Brzykcy from Jacksonville Jumbo Shrimp.
- **COL** [IL] — Colorado Rockies activated RHP Tanner Gordon from the 15-day injured list.
- **COL** [optioned] — Colorado Rockies optioned LHP Sean Sullivan to Albuquerque Isotopes.
- **CWS** [signed] — Chicago White Sox signed free agent RHP Erick Rosso to a minor league contract.
- **CWS** [IL] — Chicago White Sox activated LHP Noah Schultz from the 15-day injured list.
- **CWS** [optioned] — Chicago White Sox optioned LHP Tyler Schweitzer to Charlotte Knights.
- **STL** [status] — St. Louis Cardinals placed RHP Ryne Stanek on the paternity list.
- **STL** [IL] — St. Louis Cardinals activated RHP Ryan Fernandez from the 15-day injured list.
- **ATL** [signed] — Atlanta Braves signed free agent OF Matt Scannell to a minor league contract.
- **DET** [IL] — Detroit Tigers placed RHP Will Vest on the 15-day injured list retroactive to June 28, 2026. Right posterior elbow inflammation.
- **DET** [recalled] — Detroit Tigers recalled RHP Beau Brieske from Toledo Mud Hens.
- **TEX** [DFA] — Texas Rangers designated RHP Chris Paddack for assignment.
- **TEX** [recalled] — Texas Rangers recalled RHP Gavin Collyer from Round Rock Express.
- **TEX** [IL] — Texas Rangers placed SS Corey Seager on the 10-day injured list. Lower back inflammation.
- **TEX** [recalled] — Texas Rangers recalled 2B Josh Smith from Round Rock Express.
- **SEA** [recalled] — Seattle Mariners recalled RHP Cole Wilcox from Tacoma Rainiers.
- **SEA** [optioned] — Seattle Mariners optioned LHP Josh Simpson to Tacoma Rainiers.
- **LAD** [DFA] — Los Angeles Dodgers designated RHP Jonathan Hernández for assignment.
- **LAD** [recalled] — Los Angeles Dodgers recalled RHP Wyatt Mills from Oklahoma City Comets.

_Suppressed 4 minor/cosmetic move(s) (ASG/NUM); 0 unrecognized code-type(s) -> logs/unknown_tx_codes.log._

## 2 · Starting Pitcher Flags
- **CIN @ MIL** (away SP Chase Burns): very high K% (30.8% vs lg 22.0)
- **CIN @ MIL** (home SP Jacob Misiorowski): very high K% (37.6% vs lg 22.0)
- **MIA @ COL** (away SP TBD): NOT CONFIRMED — probable SP not posted
- **STL @ ATL** (home SP TBD): NOT CONFIRMED — probable SP not posted
- **TB @ KC** (away SP TBD): NOT CONFIRMED — probable SP not posted
- **TB @ KC** (home SP TBD): NOT CONFIRMED — probable SP not posted
- **LAA @ SEA** (home SP Bryce Miller): very high K% (31.2% vs lg 22.0)
- **SD @ LAD** (away SP TBD): NOT CONFIRMED — probable SP not posted

## 3 · Bullpen Fatigue (last 3 games)
- **MIL** Chad Patrick — 2 apps / 39 pitches over last 3 games — monitor

## 4 · Lineup / Platoon
- PIT @ PHI — PHI faces RHP (Jared Jones)
- PIT @ PHI — PIT faces RHP (Alan Rangel)
- CIN @ MIL — MIL faces RHP (Chase Burns)
- CIN @ MIL — CIN faces RHP (Jacob Misiorowski)
- MIA @ COL — MIA faces RHP (Michael Lorenzen)
- CWS @ CLE — CLE faces RHP (Davis Martin)
- CWS @ CLE — CWS faces RHP (Slade Cecconi)
- STL @ ATL — ATL faces RHP (Dustin May)
- DET @ TEX — TEX faces LHP (Framber Valdez)
- DET @ TEX — DET faces RHP (Nathan Eovaldi)
- LAA @ SEA — SEA faces RHP (Walbert Ureña)
- LAA @ SEA — LAA faces RHP (Bryce Miller)
- SD @ LAD — SD faces RHP (Roki Sasaki)

