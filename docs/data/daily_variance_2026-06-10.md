# Daily Variance Report — 2026-06-10
_Generated 2026-06-10T07:09:03Z · 15 games · significant deviations only_

## 1 · Roster / Injury Delta
- **BOS** [optioned] — Boston Red Sox optioned LHP Joe La Sorsa to Worcester Red Sox.
- **BOS** [recalled] — Boston Red Sox recalled OF Nate Eaton from Worcester Red Sox.
- **BOS** [IL] — Boston Red Sox activated RHP Garrett Whitlock from the 15-day injured list.
- **BOS** [optioned] — Boston Red Sox optioned 2B Anthony Seigler to Worcester Red Sox.
- **TB** [IL] — Tampa Bay Rays transferred RHP Jonathan Heasley from the 15-day injured list to the 60-day injured list.
- **TB** [selected] — Tampa Bay Rays selected the contract of RF Austin Slater from Durham Bulls.
- **TB** [optioned] — Tampa Bay Rays optioned 3B Oliver Dunn to Durham Bulls.
- **SD** [status] — San Diego Padres placed SS Xander Bogaerts on the paternity list.
- **SD** [recalled] — San Diego Padres recalled 3B Will Wagner from El Paso Chihuahuas.
- **SEA** [IL] — Seattle Mariners placed SS J.P. Crawford on the 10-day injured list retroactive to June 6, 2026. Right hand contusion.
- **SEA** [recalled] — Seattle Mariners recalled 2B Ryan Bliss from Tacoma Rainiers.
- **SEA** [IL] — Seattle Mariners placed RHP Cooper Criswell on the 15-day injured list. Right shoulder ctrain.
- **SEA** [recalled] — Seattle Mariners recalled RHP Nick Davila from Tacoma Rainiers.
- **BAL** [IL] — Baltimore Orioles placed RHP Chris Bassitt on the 15-day injured list retroactive to June 5, 2026. Low back discomfort.
- **BAL** [claimed] — Chicago Cubs claimed RHP Eduarniel Núñez off waivers from Baltimore Orioles.
- **BAL** [recalled] — Baltimore Orioles recalled RHP Trey Gibson from Norfolk Tides.
- **PIT** [signed] — Pittsburgh Pirates signed free agent RHP Chun Hsi Lin to a minor league contract.
- **MIN** [status] — IF Nolwer Barboza roster status changed by Minnesota Twins.
- **MIN** [signed] — Minnesota Twins signed free agent RHP Failin Placencio to a minor league contract.
- **MIN** [signed] — Minnesota Twins signed free agent SS Cody Morissette to a minor league contract.
- **MIN** [signed] — Minnesota Twins signed free agent C Nolwer Barboza to a minor league contract.
- **MIN** [signed] — Minnesota Twins signed free agent IF Nolwer Barboza to a minor league contract.
- **MIN** [TRADE] — Arizona Diamondbacks traded RHP Taylor Rashi to Minnesota Twins for cash.
- **AZ** [signed] — Arizona Diamondbacks signed free agent RHP Oscar Echegarreta to a minor league contract.
- **AZ** [TRADE] — Arizona Diamondbacks traded RHP Taylor Rashi to Minnesota Twins for cash.
- **TOR** [IL] — Toronto Blue Jays activated RHP Tommy Nance from the 15-day injured list.
- **TOR** [DFA] — Toronto Blue Jays designated RHP Yariel Rodríguez for assignment.
- **TOR** [IL] — Toronto Blue Jays activated RHP Dylan Cease from the 15-day injured list.
- **TOR** [optioned] — Toronto Blue Jays optioned LHP Adam Macko to Buffalo Bisons.
- **TOR** [outrighted] — Toronto Blue Jays sent RHP Tanner Andrews outright to Buffalo Bisons.
- **STL** [signed] — St. Louis Cardinals signed free agent RHP Andrew Schultz to a minor league contract.
- **STL** [optioned] — St. Louis Cardinals optioned CF Victor Scott II to Memphis Redbirds.
- **STL** [IL] — St. Louis Cardinals activated LF Nathan Church from the 10-day injured list.
- **NYM** [signed] — New York Mets signed free agent C Jack Scanlon to a minor league contract.
- **NYM** [optioned] — New York Mets optioned C Hayden Senger to Syracuse Mets.
- **TEX** [IL] — Texas Rangers activated RHP Cole Winn from the 15-day injured list.
- **TEX** [optioned] — Texas Rangers optioned RHP Luis Curvelo to Round Rock Express.
- **KC** [activated] — Kansas City Royals activated RHP Stephen Kolek.
- **KC** [optioned] — Kansas City Royals optioned 3B Josh Rojas to Omaha Storm Chasers.
- **CWS** [IL] — Chicago White Sox placed LF Austin Hays on the 60-day injured list. Left calf strain.
- **CWS** [selected] — Chicago White Sox selected the contract of OF Braden Montgomery from Charlotte Knights.
- **CWS** [optioned] — Chicago White Sox optioned RHP David Sandlin to Charlotte Knights.
- **CWS** [recalled] — Chicago White Sox recalled LHP Joe Rock from Charlotte Knights.
- **CWS** [optioned] — Chicago White Sox optioned RF Rikuu Nishida to Charlotte Knights.
- **CHC** [selected] — Chicago Cubs selected the contract of LHP Antoine Kelly from Iowa Cubs.
- **CHC** [claimed] — Chicago Cubs claimed RHP Eduarniel Núñez off waivers from Baltimore Orioles.
- **CHC** [IL] — Chicago Cubs placed RHP Jameson Taillon on the 15-day injured list retroactive to June 8, 2026.
- **CHC** [IL] — Chicago Cubs activated RF Matt Shaw from the 10-day injured list.
- **CHC** [status] — Chicago Cubs placed RHP Trent Thornton on the paternity list.
- **CHC** [recalled] — Chicago Cubs recalled RHP Ethan Roberts from Iowa Cubs.
- **CHC** [recalled] — Chicago Cubs recalled RHP Tyler Ferguson from Iowa Cubs.
- **CHC** [optioned] — Chicago Cubs optioned DH Kevin Alcántara to Iowa Cubs.
- **COL** [IL] — Colorado Rockies placed RF Tyler Freeman on the 7-day injured list. Concussion.
- **COL** [DFA] — Colorado Rockies designated RHP Keegan Thompson for assignment.
- **COL** [recalled] — Colorado Rockies recalled RHP Jeff Criswell from Albuquerque Isotopes.
- **COL** [selected] — Colorado Rockies selected the contract of OF Cole Carrigg from Albuquerque Isotopes.
- **MIL** [IL] — Milwaukee Brewers placed RHP Carlos Rodriguez on the 15-day injured list retroactive to May 31, 2026.
- **MIL** [activated] — Milwaukee Brewers activated RHP Joel Kuhnel.
- **MIL** [optioned] — Milwaukee Brewers optioned RHP Craig Yoho to Nashville Sounds.
- **MIL** [outrighted] — Milwaukee Brewers sent RHP Jake Woodford outright to Nashville Sounds.
- **MIL** [IL] — Milwaukee Brewers transferred LHP Brian Fitzpatrick from the 15-day injured list to the 60-day injured list. Left elbow injury.
- **MIL** [selected] — Milwaukee Brewers selected the contract of CF Luis Lara from Nashville Sounds.
- **MIL** [optioned] — Milwaukee Brewers optioned CF Luis Lara to Nashville Sounds.
- **ATH** [IL] — Athletics activated 3B Max Muncy from the 10-day injured list.
- **ATH** [optioned] — Athletics optioned SS Darell Hernaiz to Las Vegas Aviators.
- **ATH** [signed] — Athletics signed free agent SS Kenny Fermin to a minor league contract.
- **ATH** [optioned] — Athletics optioned RHP Kade Morris to Las Vegas Aviators.
- **ATH** [recalled] — Athletics recalled RHP Joey Estes from Las Vegas Aviators.
- **HOU** [signed] — Houston Astros signed free agent LF Trenton Brooks to a minor league contract.
- **HOU** [IL] — Houston Astros placed SS Nick Allen on the 10-day injured list. Left hamstring strain.
- **HOU** [recalled] — Houston Astros recalled 3B Shay Whitcomb from Sugar Land Space Cowboys.
- **HOU** [IL] — Houston Astros placed 1B LaMonte Wade Jr. on the 10-day injured list. Right hamstring strain.
- **HOU** [recalled] — Houston Astros recalled LF Joey Loperfido from Sugar Land Space Cowboys.
- **LAA** [IL] — Los Angeles Angels placed 2B Vaughn Grissom on the 10-day injured list retroactive to June 5, 2026. Left oblique strain.
- **LAA** [IL] — Los Angeles Angels transferred 3B Yoán Moncada from the 10-day injured list to the 60-day injured list. Right knee inflammation.
- **LAA** [signed] — Los Angeles Angels signed free agent RHP Shaun Anderson to a minor league contract.
- **LAA** [IL] — Los Angeles Angels placed 2B Adam Frazier on the 10-day injured list retroactive to June 6, 2026. Right elbow inflammation.
- **LAA** [signed] — Los Angeles Angels signed free agent C Logan Porter to a minor league contract.
- **LAA** [recalled] — Los Angeles Angels recalled SS Denzer Guzman from Salt Lake Bees.
- **LAA** [selected] — Los Angeles Angels selected the contract of 1B Trey Mancini from Salt Lake Bees.

## 1b · Unrecognized codes (review — kept, never dropped)
- **BOS** code=ACQ — Boston Red Sox acquired LHP Tyler Davis from the Washington Wild Things of the Frontier League.
- **WSH** code=ACQ — Washington Nationals acquired RHP Brock Gillis from the Yuba-Sutter Freebirds of the Pioneer League.
- **CIN** code=ACQ — Cincinnati Reds acquired LHP Steffon Moore from the Kansas City Monarchs of the American Association.

_Suppressed 9 minor/cosmetic move(s) (ASG/NUM); 1 unrecognized code-type(s) -> logs/unknown_tx_codes.log._

## 2 · Starting Pitcher Flags
- **LAD @ PIT** (away SP Shohei Ohtani): very high K% (30.9% vs lg 21.7)
- **MIN @ DET** (away SP TBD): NOT CONFIRMED — probable SP not posted
- **ATL @ CWS** (home SP TBD): NOT CONFIRMED — probable SP not posted
- **HOU @ LAA** (home SP Reid Detmers): very high K% (29.4% vs lg 21.7)

## 3 · Bullpen Fatigue (last 3 games)
- **TB** Garrett Cleavinger — 2 apps / 36 pitches over last 3 games — monitor
- **NYY** Fernando Cruz — 2 apps / 35 pitches over last 3 games — monitor
- **NYY** Tim Hill — overused — 3 apps / 22 pitches over last 3 games
- **NYY** David Bednar — 2 apps / 38 pitches over last 3 games — monitor
- **NYY** Paul Blackburn — 2 apps / 38 pitches over last 3 games — monitor
- **WSH** Brad Lord — threw 57 pitches last night — likely unavailable
- **SF** JT Brubaker — 2 apps / 38 pitches over last 3 games — monitor
- **SF** Dylan Smith — overused — 3 apps / 37 pitches over last 3 games
- **SF** Keaton Winn — 2 apps / 41 pitches over last 3 games — monitor
- **SD** Adrian Morejon — 2 apps / 47 pitches over last 3 games — monitor
- **SD** Jason Adam — 2 apps / 44 pitches over last 3 games — monitor
- **SD** Yuki Matsui — 2 apps / 39 pitches over last 3 games — monitor
- **SEA** José A. Ferrer — 2 apps / 60 pitches over last 3 games — monitor
- **SEA** Andrés Muñoz — 2 apps / 39 pitches over last 3 games — monitor
- **LAD** Blake Treinen — 2 apps / 53 pitches over last 3 games — monitor
- **MIN** Cody Laweryson — 2 apps / 48 pitches over last 3 games — monitor
- **MIN** Justin Lawrence — 2 apps / 38 pitches over last 3 games — monitor
- **DET** Kyle Finnegan — 2 apps / 39 pitches over last 3 games — monitor
- **MIA** Michael Petersen — 2 apps / 43 pitches over last 3 games — monitor
- **MIA** Anthony Bender — 2 apps / 35 pitches over last 3 games — monitor
- **PHI** Orion Kerkering — 2 apps / 49 pitches over last 3 games — monitor
- **PHI** José Alvarado — 2 apps / 40 pitches over last 3 games — monitor
- **TOR** Adam Macko — 2 apps / 37 pitches over last 3 games — monitor
- **CWS** Erick Fedde — threw 88 pitches last night — likely unavailable
- **COL** Jaden Hill — 2 apps / 36 pitches over last 3 games — monitor
- **MIL** Joel Kuhnel — 2 apps / 43 pitches over last 3 games — monitor
- **ATH** Mark Leiter Jr. — 2 apps / 39 pitches over last 3 games — monitor
- **ATH** Hogan Harris — 2 apps / 51 pitches over last 3 games — monitor
- **LAA** Kirby Yates — 2 apps / 36 pitches over last 3 games — monitor

## 4 · Lineup / Platoon
- BOS @ TB — TB faces LHP (Jake Bennett)
- BOS @ TB — BOS faces RHP (Drew Rasmussen)
- NYY @ CLE — CLE faces LHP (Carlos Rodón)
- NYY @ CLE — NYY faces LHP (Parker Messick)
- WSH @ SF — SF faces LHP (Foster Griffin)
- WSH @ SF — WSH faces LHP (Robbie Ray)
- CIN @ SD — SD faces RHP (Brady Singer)
- CIN @ SD — CIN faces RHP (Michael King)
- SEA @ BAL — BAL faces RHP (George Kirby)
- SEA @ BAL — SEA faces RHP (Brandon Young)
- LAD @ PIT — PIT faces RHP (Shohei Ohtani)
- LAD @ PIT — LAD faces RHP (Jared Jones)
- MIN @ DET — MIN faces LHP (Framber Valdez)
- AZ @ MIA — MIA faces RHP (Ryne Nelson)
- AZ @ MIA — AZ faces RHP (Ryan Gusto)
- PHI @ TOR — TOR faces LHP (Jesús Luzardo)
- PHI @ TOR — PHI faces RHP (Max Scherzer)
- STL @ NYM — NYM faces RHP (Andre Pallante)
- STL @ NYM — STL faces RHP (Austin Warren)
- TEX @ KC — KC faces LHP (MacKenzie Gore)
- TEX @ KC — TEX faces RHP (Seth Lugo)
- ATL @ CWS — CWS faces LHP (Chris Sale)
- CHC @ COL — COL faces LHP (Shota Imanaga)
- CHC @ COL — CHC faces RHP (Michael Lorenzen)
- MIL @ ATH — ATH faces RHP (Brandon Sproat)
- MIL @ ATH — MIL faces RHP (Jack Perkins)
- HOU @ LAA — LAA faces RHP (Peter Lambert)
- HOU @ LAA — HOU faces LHP (Reid Detmers)

