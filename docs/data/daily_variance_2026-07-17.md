# Daily Variance Report — 2026-07-17
_Generated 2026-07-17T02:57:20Z · 15 games · significant deviations only_

## 1 · Roster / Injury Delta
- **PIT** [status] — CF Robert Hassell III roster status changed by Pittsburgh Pirates.
- **PIT** [TRADE] — Washington Nationals traded CF Robert Hassell III to Pittsburgh Pirates for cash.
- **CWS** [signed] — Chicago White Sox signed RHP Eric Segura.
- **CWS** [signed] — Chicago White Sox signed free agent RHP Nathan Leininger to a minor league contract.
- **CWS** [signed] — Chicago White Sox signed OF Matthew Bucciero.
- **CWS** [signed] — Chicago White Sox signed free agent LHP Grant Cleavinger to a minor league contract.
- **CWS** [signed] — Chicago White Sox signed RHP Joey Volchko.
- **CWS** [signed] — Chicago White Sox signed SS Roch Cholowsky.
- **CWS** [signed] — Chicago White Sox signed RHP Kyle Casteel.
- **CWS** [signed] — Chicago White Sox signed 1B Grayson Fitzwater.
- **CWS** [signed] — Chicago White Sox signed SS Jake Berkland.
- **CWS** [signed] — Chicago White Sox signed RHP Isaac Yeager.
- **CWS** [signed] — Chicago White Sox signed 3B Jayson Jones.
- **CWS** [signed] — Chicago White Sox signed free agent RHP Justin Feld to a minor league contract.
- **CWS** [signed] — Chicago White Sox signed RHP Jackson Bergman.
- **CWS** [signed] — Chicago White Sox signed free agent LHP Michael Sharman to a minor league contract.
- **CWS** [signed] — Chicago White Sox signed OF Crew McChesney.
- **CWS** [signed] — Chicago White Sox signed RHP Cal Scolari.
- **CWS** [signed] — Chicago White Sox signed free agent 3B Luke Lawrence to a minor league contract.
- **CWS** [signed] — Chicago White Sox signed LHP Cameron Johnson.
- **CWS** [signed] — Chicago White Sox signed free agent RHP Andrew Berg to a minor league contract.
- **CWS** [signed] — Chicago White Sox signed RHP Darin Horn.
- **CWS** [signed] — Chicago White Sox signed SS Landon Thome.
- **CWS** [signed] — Chicago White Sox signed OF Alex Weingartner.
- **CWS** [signed] — Chicago White Sox signed RHP Luke Craytor.
- **CWS** [signed] — Chicago White Sox signed OF Clay Burdette.
- **CWS** [signed] — Chicago White Sox signed OF Braden Holcomb.
- **CWS** [signed] — Chicago White Sox signed 2B Cole Prosek.
- **MIL** [IL] — Milwaukee Brewers placed LHP Rob Zastryzny on the 60-day injured list. Left trapezius strain.
- **MIL** [optioned] — Milwaukee Brewers optioned LHP Colton Gordon to Nashville Sounds.
- **MIL** [TRADE] — Houston Astros traded RHP Lance McCullers Jr., LHP Colton Gordon and cash to Milwaukee Brewers for OF Jadyn Fielder.
- **BAL** [recalled] — Baltimore Orioles recalled 1B Christian Encarnacion-Strand from Norfolk Tides.
- **HOU** [TRADE] — Houston Astros traded RHP Lance McCullers Jr., LHP Colton Gordon and cash to Milwaukee Brewers for OF Jadyn Fielder.
- **SD** [signed] — San Diego Padres signed free agent RHP Jackson Vasquez to a minor league contract.
- **SD** [status] — San Diego Padres placed LHP Wandy Peralta on the bereavement list.
- **SD** [recalled] — San Diego Padres recalled RHP Jhony Brito from El Paso Chihuahuas.
- **WSH** [IL] — Washington Nationals placed C Drew Millas on the 10-day injured list retroactive to July 12, 2026. Left index finger fracture.
- **WSH** [IL] — Washington Nationals activated RHP Max Kranick from the 60-day injured list.
- **WSH** [TRADE] — Washington Nationals traded CF Robert Hassell III to Pittsburgh Pirates for cash.
- **ATH** [DFA] — Athletics designated RHP Aaron Civale for assignment.
- **ATH** [activated] — Athletics activated 3B Donovan Walton.
- **ATH** [optioned] — Athletics optioned RHP Mason Barnett to Las Vegas Aviators.
- **ATH** [optioned] — Athletics optioned C Brian Serven to Las Vegas Aviators.
- **ATH** [recalled] — Athletics recalled LHP Brady Basso from Las Vegas Aviators.
- **ATH** [selected] — Athletics selected the contract of RHP Yunior Tur from Las Vegas Aviators.
- **ATH** [optioned] — Athletics optioned 1B Joey Meneses to Las Vegas Aviators.
- **STL** [signed] — St. Louis Cardinals signed free agent RHP Rayner Reyes to a minor league contract.
- **AZ** [IL] — Arizona Diamondbacks placed LF Tommy Troy on the 10-day injured list retroactive to July 12, 2026. Sprained right AC joint.
- **AZ** [outrighted] — Arizona Diamondbacks sent 1B Pavin Smith outright to Reno Aces.
- **AZ** [outrighted] — Arizona Diamondbacks sent 1B Pavin Smith outright to Reno Aces.
- **SF** [outrighted] — San Francisco Giants sent RHP Eric Cerantola outright to Sacramento River Cats.

_Suppressed 1 minor/cosmetic move(s) (ASG/NUM); 0 unrecognized code-type(s) -> logs/unknown_tx_codes.log._

## 2 · Starting Pitcher Flags
- **TB @ BOS** (away SP TBD): NOT CONFIRMED — probable SP not posted
- **TB @ BOS** (home SP TBD): NOT CONFIRMED — probable SP not posted
- **PIT @ CLE** (home SP Gavin Williams): very high K% (29.8% vs lg 22.1)
- **STL @ AZ** (away SP TBD): NOT CONFIRMED — probable SP not posted
- **SF @ SEA** (home SP Bryce Miller): very high K% (30.2% vs lg 22.1)

## 3 · Bullpen Fatigue (last 3 games)
_None flagged._

## 4 · Lineup / Platoon
- TB @ BOS — BOS faces RHP (Griffin Jax)
- TB @ BOS — TB faces LHP (Jake Bennett)
- LAD @ NYY — NYY faces RHP (Roki Sasaki)
- LAD @ NYY — LAD faces RHP (Gerrit Cole)
- PIT @ CLE — CLE faces RHP (Jared Jones)
- PIT @ CLE — PIT faces RHP (Gavin Williams)
- TEX @ ATL — ATL faces RHP (Cal Quantrill)
- TEX @ ATL — TEX faces LHP (Chris Sale)
- CWS @ TOR — TOR faces LHP (Anthony Kay)
- CWS @ TOR — CWS faces RHP (Spencer Miles)
- MIA @ MIL — MIL faces RHP (Sandy Alcantara)
- MIA @ MIL — MIA faces RHP (Logan Henderson)
- MIN @ CHC — CHC faces RHP (Bailey Ober)
- MIN @ CHC — MIN faces RHP (Colin Rea)
- BAL @ HOU — HOU faces RHP (Dean Kremer)
- BAL @ HOU — BAL faces RHP (Peter Lambert)
- SD @ KC — KC faces RHP (Michael King)
- SD @ KC — SD faces RHP (Seth Lugo)
- CIN @ COL — COL faces RHP (Brady Singer)
- CIN @ COL — CIN faces RHP (Gabriel Hughes)
- DET @ LAA — LAA faces RHP (Troy Melton)
- DET @ LAA — DET faces LHP (Reid Detmers)
- WSH @ ATH — ATH faces RHP (Cade Cavalli)
- WSH @ ATH — WSH faces LHP (Gage Jump)
- STL @ AZ — STL faces RHP (Merrill Kelly)
- SF @ SEA — SEA faces RHP (Landen Roupp)
- SF @ SEA — SF faces RHP (Bryce Miller)

