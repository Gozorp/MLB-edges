# Daily Variance Report — 2026-06-30
_Generated 2026-06-29T22:19:50Z · 15 games · significant deviations only_

## 1 · Roster / Injury Delta
- **BAL** [IL] — Baltimore Orioles activated C Adley Rutschman from the 7-day injured list.
- **BAL** [IL] — Baltimore Orioles activated RF Dylan Beavers from the 10-day injured list.
- **BAL** [DFA] — Baltimore Orioles designated C Sam Huff for assignment.
- **BAL** [optioned] — Baltimore Orioles optioned RHP Yosver Zulueta to Norfolk Tides.
- **BAL** [optioned] — Baltimore Orioles optioned 2B Jeremiah Jackson to Norfolk Tides.
- **BAL** [claimed] — Baltimore Orioles claimed RHP Yosver Zulueta off waivers from Chicago Cubs.
- **TEX** [IL] — Texas Rangers activated CF Evan Carter from the 10-day injured list.
- **TEX** [IL] — Texas Rangers placed LF Wyatt Langford on the 10-day injured list. Left hamstring strain.
- **TEX** [signed] — Texas Rangers signed free agent RHP Chris Paddack.
- **TEX** [DFA] — Texas Rangers designated RF Jarred Kelenic for assignment.
- **TEX** [DFA] — Texas Rangers designated RHP Joe Ross for assignment.
- **TEX** [selected] — Texas Rangers selected the contract of SS Cameron Cauley from Round Rock Express.
- **CLE** [IL] — Cleveland Guardians activated RF Chase DeLauter from the 10-day injured list.
- **CLE** [optioned] — Cleveland Guardians optioned CF Petey Halpin to Columbus Clippers.
- **DET** [IL] — Detroit Tigers activated RHP Jack Flaherty from the 15-day injured list.
- **DET** [optioned] — Detroit Tigers optioned LHP Enmanuel De Jesus to Toledo Mud Hens.
- **NYY** [status] — New York Yankees placed RHP David Bednar on the paternity list.
- **NYY** [recalled] — New York Yankees recalled RHP Jake Bird from Scranton/Wilkes-Barre RailRiders.
- **NYM** [optioned] — New York Mets optioned RHP Tobias Myers to Syracuse Mets.
- **NYM** [recalled] — New York Mets recalled RHP Joey Gerber from Syracuse Mets.
- **WSH** [IL] — Washington Nationals placed LHP Richard Lovelady on the 15-day injured list. Left triceps strain.
- **WSH** [IL] — Washington Nationals placed LHP Mitchell Parker on the 15-day injured list. Left elbow inflammation.
- **WSH** [signed] — Washington Nationals signed free agent LHP Kai Peterson to a minor league contract.
- **WSH** [recalled] — Washington Nationals recalled RHP Riley Cornelio from Rochester Red Wings.
- **WSH** [outrighted] — Washington Nationals sent RHP Jhancarlos Lara outright to Harrisburg Senators.
- **WSH** [recalled] — Washington Nationals recalled LHP Carson Palmquist from Rochester Red Wings.
- **BOS** [IL] — Boston Red Sox activated 1B Romy Gonzalez from the 60-day injured list.
- **BOS** [IL] — Boston Red Sox transferred LF Roman Anthony from the 10-day injured list to the 60-day injured list.
- **BOS** [optioned] — Boston Red Sox optioned C Mickey Gasper to Worcester Red Sox.
- **ATL** [signed] — Atlanta Braves signed free agent SS Junior Tilien to a minor league contract.
- **ATL** [signed] — Atlanta Braves signed free agent OF Cardell Thibodeaux to a minor league contract.
- **TB** [optioned] — Tampa Bay Rays optioned RHP Michael Grove to Durham Bulls.
- **TB** [recalled] — Tampa Bay Rays recalled RHP Trevor Martin from Durham Bulls.
- **CIN** [IL] — Cincinnati Reds placed CF Blake Dunn on the 10-day injured list retroactive to June 27, 2026. Right elbow sprain.
- **CIN** [recalled] — Cincinnati Reds recalled CF TJ Friedl from Louisville Bats.
- **SD** [IL] — San Diego Padres placed RHP David Morgan on the 15-day injured list retroactive to June 28, 2026. Left knee inflammation.
- **SD** [IL] — San Diego Padres activated 2B Jake Cronenworth from the 7-day injured list.
- **SD** [recalled] — San Diego Padres recalled RHP Bradgley Rodriguez from San Antonio Missions.
- **SD** [optioned] — San Diego Padres optioned DH Will Wagner to El Paso Chihuahuas.
- **CHC** [DFA] — Chicago Cubs designated RHP Vince Velasquez for assignment.
- **CHC** [recalled] — Chicago Cubs recalled LHP Jordan Wicks from Iowa Cubs.
- **CHC** [claimed] — Baltimore Orioles claimed RHP Yosver Zulueta off waivers from Chicago Cubs.
- **MIN** [IL] — Minnesota Twins placed LHP Anthony Banda on the 15-day injured list. Left lat strain.
- **MIN** [recalled] — Minnesota Twins recalled RHP Cody Laweryson from St. Paul Saints.
- **HOU** [optioned] — Houston Astros optioned RHP Kai-Wei Teng to Sugar Land Space Cowboys.
- **HOU** [recalled] — Houston Astros recalled RHP Miguel Ullola from Sugar Land Space Cowboys.
- **HOU** [IL] — Houston Astros activated SS Nick Allen from the 10-day injured list.
- **HOU** [optioned] — Houston Astros optioned 2B Raynel Delgado to Sugar Land Space Cowboys.
- **COL** [IL] — Colorado Rockies activated LF Jordan Beck from the 10-day injured list.
- **COL** [optioned] — Colorado Rockies optioned LF Jordan Beck to Albuquerque Isotopes.
- **LAD** [IL] — Los Angeles Dodgers activated LF Teoscar Hernández from the 10-day injured list.
- **LAD** [optioned] — Los Angeles Dodgers optioned LF Ryan Ward to Oklahoma City Comets.
- **ATH** [IL] — Athletics placed LF Tyler Soderstrom on the 10-day injured list retroactive to June 28, 2026. Left hip impingement.
- **ATH** [IL] — Athletics placed SS Jacob Wilson on the 10-day injured list retroactive to June 26, 2026. Right thumb inflammation.
- **ATH** [DFA] — Athletics designated RHP Michael Kelly for assignment.
- **ATH** [status] — Athletics placed LHP José Suarez on the paternity list.
- **ATH** [recalled] — Athletics recalled RHP Kade Morris from Las Vegas Aviators.
- **ATH** [selected] — Athletics selected the contract of SS Joshua Kuroda-Grauer from Las Vegas Aviators.
- **ATH** [recalled] — Athletics recalled SS Darell Hernaiz from Las Vegas Aviators.
- **SEA** [TRADE] — San Francisco Giants traded 1B Buddy Kennedy to Seattle Mariners for cash.
- **SF** [DFA] — San Francisco Giants designated 1B Buddy Kennedy for assignment.
- **SF** [IL] — San Francisco Giants activated LF Heliot Ramos from the 10-day injured list.
- **SF** [TRADE] — San Francisco Giants traded 1B Buddy Kennedy to Seattle Mariners for cash.

## 1b · Unrecognized codes (review — kept, never dropped)
- **LAA** code=ACQ — Los Angeles Angels acquired RHP Zach Kirby from the Washington Wild Things of the Frontier League.

_Suppressed 6 minor/cosmetic move(s) (ASG/NUM); 1 unrecognized code-type(s) -> logs/unknown_tx_codes.log._

## 2 · Starting Pitcher Flags
- **TEX @ CLE** (away SP Jacob deGrom): very high K% (29.6% vs lg 22.0)
- **PIT @ PHI** (home SP Cristopher Sánchez): very high K% (28.2% vs lg 22.0)
- **DET @ NYY** (home SP Cam Schlittler): very high K% (28.9% vs lg 22.0)
- **NYM @ TOR** (away SP Nolan McLean): very high K% (28.4% vs lg 22.0)
- **SD @ CHC** (away SP JP Sears): thin sample / PENDING_SP_DATA — JP Sears has only 81 Statcast pitches season-to-date; need 100+ to score
- **MIA @ COL** (home SP TBD): NOT CONFIRMED — probable SP not posted
- **SF @ AZ** (home SP TBD): NOT CONFIRMED — probable SP not posted

## 3 · Bullpen Fatigue (last 3 games)
- **CLE** Cade Smith — 2 apps / 35 pitches over last 3 games — monitor
- **PHI** Kyle Backhus — 2 apps / 35 pitches over last 3 games — monitor
- **TOR** Jeff Hoffman — 2 apps / 44 pitches over last 3 games — monitor
- **WSH** PJ Poulin — 2 apps / 36 pitches over last 3 games — monitor
- **BOS** Aroldis Chapman — 2 apps / 43 pitches over last 3 games — monitor
- **MIL** Joel Kuhnel — 2 apps / 44 pitches over last 3 games — monitor
- **HOU** Josh Hader — 2 apps / 40 pitches over last 3 games — monitor
- **LAA** Ryan Zeferjahn — 2 apps / 35 pitches over last 3 games — monitor

## 4 · Lineup / Platoon
- CWS @ BAL — BAL faces RHP (Erick Fedde)
- CWS @ BAL — CWS faces RHP (Trey Gibson)
- TEX @ CLE — CLE faces RHP (Jacob deGrom)
- TEX @ CLE — TEX faces RHP (Tanner Bibee)
- PIT @ PHI — PHI faces RHP (Bubba Chandler)
- PIT @ PHI — PIT faces LHP (Cristopher Sánchez)
- DET @ NYY — NYY faces LHP (Tarik Skubal)
- DET @ NYY — DET faces RHP (Cam Schlittler)
- NYM @ TOR — TOR faces RHP (Nolan McLean)
- NYM @ TOR — NYM faces RHP (Kevin Gausman)
- WSH @ BOS — BOS faces RHP (Cade Cavalli)
- WSH @ BOS — WSH faces LHP (Connelly Early)
- STL @ ATL — ATL faces LHP (Matthew Liberatore)
- STL @ ATL — STL faces LHP (Martín Pérez)
- TB @ KC — KC faces RHP (Griffin Jax)
- TB @ KC — TB faces LHP (Noah Cameron)
- CIN @ MIL — MIL faces RHP (Rhett Lowder)
- CIN @ MIL — CIN faces RHP (Brandon Sproat)
- SD @ CHC — CHC faces LHP (JP Sears)
- SD @ CHC — SD faces LHP (Matthew Boyd)
- MIN @ HOU — HOU faces RHP (Joe Ryan)
- MIN @ HOU — MIN faces RHP (Mike Burrows)
- MIA @ COL — COL faces RHP (Eury Pérez)
- LAD @ ATH — ATH faces LHP (Justin Wrobleski)
- LAD @ ATH — LAD faces LHP (Jeffrey Springs)
- LAA @ SEA — SEA faces RHP (José Soriano)
- LAA @ SEA — LAA faces RHP (Bryan Woo)
- SF @ AZ — AZ faces RHP (Landen Roupp)

