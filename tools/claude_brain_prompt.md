...
- **[HARD CAP 8] Bottom-bucket marginal-favorite filter** (2026-05-20, re-ship 2026-05-21) — when `pick_prob` lands in `(0.50, 0.52]` AND `edge_pp <= +12pp` AND the pick is tier-elevated (score >= 3), force score=1 (B-, do not parlay). Grid-derived from the 9-day archive (n=113 picks): the (0.50, 0.55] band holds 57% of slate and hits 40.6% actual vs 52.4% predicted; the narrowest sub-band (0.50, 0.52] is the highest-leak archetype. LODO stable in 7/8 folds; OVERFITTING RISK acknowledged — postgame cron monitors prospective fires and removes cap if 30+ live captures regress above 50% hit rate.
- **[HARD CAP 9] Top-bucket calibrator hallucination** (2026-05-21) — any `pick_prob > 0.80` forces score=0 (SKIP). Archive postgame audit: the (0.80, 1.00] bucket hit 33.3% actual vs 87.3% predicted (-53.9pp catastrophic miscalibration). HARD CAP 6 catches the >+25pp edge subset; CAP 9 catches everything else in the top probability bucket. Cheap insurance until weekly calibrator retraining lands. Do NOT relax until the retrained isotonic calibrator demonstrates well-calibrated upper-bucket output on a fresh holdout.
- **[HARD CAP 10] Bottom-bucket SKIP-unless-CONFIRM** (2026-05-21) — when `pick_prob` lands in `(0.50, 0.55]` AND `claude_decision` does NOT contain `CONFIRM`, force score=0 (SKIP). Probe of 59 joined picks (May 9–18): CONFIRM-rescued picks in the band hit 47.6% (+16pp over non-CONFIRM); non-CONFIRM picks in the band hit 31.2%. The rule preserves the CONFIRM upside while killing the structural-loss non-CONFIRM slice. NOTE: even CONFIRM-rescued picks in this band hit below 50% breakeven — a more aggressive variant (universal SKIP across (0.50, 0.55] regardless of decision) is documented as a candidate tightening if production performance does not improve over the next 30 graded slates. When operating today, treat CONFIRM in this band as a low-conviction stake-down candidate, not a high-confidence bet.
